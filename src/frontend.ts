...
export function setup(ctx: SpindleFrontendContext) {
  ...
  let draftConfigs: Record<string, NudgeConfig> = {}
  ...
  function avatarUrl(imageId: string | null): string | null {
    return imageId ? `/api/v1/images/${imageId}?size=sm` : null
  }

  function getDraft(charId: string): NudgeConfig {
    if (!draftConfigs[charId]) {
      const base = configs[charId] ?? getDefaultConfig()
      const defaults = getDefaultConfig()
      // Backfill any fields missing from old configs
      draftConfigs[charId] = {
        ...defaults,
        ...base,
...
Stay fully in character. Be creative — sometimes playful, sometimes sincere, sometimes a little pouty or worried. Do NOT use quotation marks around the message. Do NOT prefix with "{{char}}:" or any name tag. Respond with ONLY the in-character message, nothing else.]`
  ...
  function getDefaultConfig(): NudgeConfig {
    return {
      enabled: false, chatId: 'most_recent', connectionId: null,
      minMinutes: 15, maxMinutes: 60, messageCount: 5,
      maxTokens: 8192, temperature: 1, topP: 0.95,
  ...
  function render() {
    ...
    if (!permissions.hasPush || !permissions.hasGeneration || !permissions.hasCharacters || !permissions.hasChats) {
      ...
      if (!permissions.hasPush) missing.push('Push Notifications')
      if (!permissions.hasGeneration) missing.push('Generation')
      if (!permissions.hasCharacters) missing.push('Characters')
      if (!permissions.hasChats) missing.push('Chats')
      if (!permissions.hasChatMutation) missing.push('Chat Mutation')
    ...
    searchInput.addEventListener('input', () => {
      ...
      const newInput = tab.root.querySelector('.cn-search input') as HTMLInputElement
      if (newInput) {
        newInput.focus()
    ...
    // Virtualize: only inflate character cards when they enter the viewport.
    // Collapsed cards are ~50px; we use a sentinel per item and an
    // IntersectionObserver with a generous rootMargin for pre-loading.
      ...
      (entries) => {
        for (const entry of entries) {
          ...
          if (entry.isIntersecting && el.dataset.cnVirt === 'placeholder') {
            ...
            el.replaceWith(real)
            activeObserver?.observe(real) // re-observe so it can be recycled if needed
          }
        }
      },
    ...
    for (let i = 0; i < sorted.length; i++) {
      // Expanded item always renders fully; others get placeholders
  ...
  function renderCharacterItem(char: CharacterInfo): HTMLElement {
  ...
  function renderCharacterBody(char: CharacterInfo): HTMLElement {
    ...
    body.appendChild(makeSelect('Chat', 'Which chat to pull context from',
      [{ value: 'most_recent', label: 'Most Recent' }, ...chats.map(c => ({ value: c.id, label: c.name || `Chat ${c.id.slice(0, 8)}` }))],
      draft.chatId, (v) => { draft.chatId = v }))

    // Connection
    body.appendChild(makeSelect('Connection', 'LLM connection for nudge generation',
      [{ value: '', label: '(Default)' }, ...connections.map(c => ({ value: c.id, label: `${c.name} (${c.provider}/${c.model})` }))],
      draft.connectionId ?? '', (v) => { draft.connectionId = v || null }))
    ...
    const minIn = makeNumericInput(draft.minMinutes, 1, 1440, 1, (v) => { draft.minMinutes = v })
    const sep = document.createElement('span')
    sep.className = 'cn-range-sep'
    sep.textContent = 'to'
    const maxIn = makeNumericInput(draft.maxMinutes, 1, 1440, 1, (v) => { draft.maxMinutes = v })
    ...
    msgField.appendChild(makeNumericInput(draft.messageCount, 1, 20, 1, (v) => { draft.messageCount = v }, true))
    ...
    sampRow.appendChild(makeLabeledInput('Max Tokens', draft.maxTokens, 1, 131072, 1, (v) => { draft.maxTokens = v }))
    sampRow.appendChild(makeLabeledInput('Temp', draft.temperature, 0, 2, 0.05, (v) => { draft.temperature = v }))
    sampRow.appendChild(makeLabeledInput('Top P', draft.topP, 0, 1, 0.05, (v) => { draft.topP = v }))
    ...
    // System prompt
    body.appendChild(makePromptEditor('System Prompt', 'Character identity for the system message.',
      draft.systemPrompt, (v) => { draft.systemPrompt = v }, defaultPrompts?.systemPrompt))

    // Nudge instruction
    body.appendChild(makePromptEditor('Nudge Instruction', 'Final instruction after chat context.',
      draft.nudgeInstruction, (v) => { draft.nudgeInstruction = v }, defaultPrompts?.nudgeInstruction))
    ...
    saveBtn.addEventListener('click', () => {
      ...
      if (d.minMinutes > d.maxMinutes) [d.minMinutes, d.maxMinutes] = [d.maxMinutes, d.minMinutes]
  ...
  function makeSelect(label: string, sub: string, options: { value: string; label: string }[], current: string, onChange: (v: string) => void): HTMLElement {
  ...
  function makeNumericInput(value: number, min: number, max: number, step: number, onChange: (v: number) => void, fullWidth = false): HTMLInputElement {
    ...
    if (!fullWidth) inp.style.width = '60px'
  ...
  function makeLabeledInput(label: string, value: number, min: number, max: number, step: number, onChange: (v: number) => void): HTMLElement {
    const field = document.createElement('div')
    field.className = 'cn-field'
  ...
  function makePromptEditor(label: string, sub: string, value: string, onChange: (v: string) => void, defaultValue?: string): HTMLElement {
    const field = document.createElement('div')
    field.className = 'cn-field'
    ...
    switch (payload.type) {
      ...
      case 'text_editor_result':
        ...
        if (expandedCharacterId) {
          ...
          if (payload.title === 'System Prompt') draft.systemPrompt = payload.text
          else if (payload.title === 'Nudge Instruction') draft.nudgeInstruction = payload.text
          render()
        }
  ...
  let globals: NudgeConfig | null = null
  let draftGlobals: Partial<NudgeConfig> = {}
  ...
  function renderSettings() {
    ...
    // Connection
    panel.appendChild(makeSelect('Default Connection', 'LLM connection for nudge generation',
      [{ value: '', label: '(Default)' }, ...connections.map(c => ({ value: c.id, label: `${c.name} (${c.provider}/${c.model})` }))],
      (g.connectionId ?? globals.connectionId) ?? '', (v) => { g.connectionId = v || null }))
    ...
    intRow.className = 'cn-range-row'
    intRow.append(
      makeNumericInput(g.minMinutes ?? globals.minMinutes, 1, 1440, 1, (v) => { g.minMinutes = v }),
      (() => { const s = document.createElement('span'); s.className = 'cn-range-sep'; s.textContent = 'to'; return s })(),
      makeNumericInput(g.maxMinutes ?? globals.maxMinutes, 1, 1440, 1, (v) => { g.maxMinutes = v }),
      (() => { const s = document.createElement('span'); s.className = 'cn-range-sep'; s.textContent = 'min'; return s })(),
    )
    intervalField.appendChild(intRow)
    ...
    msgField.appendChild(makeNumericInput(g.messageCount ?? globals.messageCount, 1, 20, 1, (v) => { g.messageCount = v }, true))
    ...
    const sampRow = document.createElement('div')
    sampRow.className = 'cn-sampler-row'
    sampRow.appendChild(makeLabeledInput('Max Tokens', g.maxTokens ?? globals.maxTokens, 1, 131072, 1, (v) => { g.maxTokens = v }))
    sampRow.appendChild(makeLabeledInput('Temp', g.temperature ?? globals.temperature, 0, 2, 0.05, (v) => { g.temperature = v }))
    sampRow.appendChild(makeLabeledInput('Top P', g.topP ?? globals.topP, 0, 1, 0.05, (v) => { g.topP = v }))
    panel.appendChild(sampRow)

    panel.appendChild(makeDivider())

    // Prompts
    panel.appendChild(makePromptEditor('Default System Prompt', 'Applied to characters without a custom system prompt.',
      g.systemPrompt ?? globals.systemPrompt, (v) => { g.systemPrompt = v }, defaultPrompts?.systemPrompt))

    panel.appendChild(makePromptEditor('Default Nudge Instruction', 'Applied to characters without a custom nudge instruction.',
      g.nudgeInstruction ?? globals.nudgeInstruction, (v) => { g.nudgeInstruction = v }, defaultPrompts?.nudgeInstruction))
  ...
  const unsubAction = nudgeAction.onClick(() => { tab.activate() })
